...
def render_single(name: str, model: type[BaseModel]) -> str:
    # Serialization mode: these schemas describe data on the wire, where a
    # datetime is an ISO string rather than a Python object.
    return _render(model.model_json_schema(mode="serialization"), name)


def render_bundle() -> str:
    """All three contracts in one document with a shared `$defs` block."""
    _, combined = models_json_schema(
        [(model, "serialization") for model in EXPORTS.values()],
...
def artifacts() -> dict[str, str]:
    rendered = {f"{name}.schema.json": render_single(name, model) for name, model in EXPORTS.items()}
